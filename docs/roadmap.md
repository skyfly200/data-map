...
- `WANT-12` **Unify MaxEnt overlay paths** — two separate rendering paths exist for suitability surfaces: `useModelOverlay` (transient handoff from `/jobs`) and `maxentLayerSpecs` (saved models in the Layer Manager, currently stub/non-rendering). Collapse into one: toggle a `maxent:*` Layer Manager entry on and mint its tile template via `useEeTiles.template()` from the `suitability_asset_path`; `openModelOnMap()` sets the active model ID and the map auto-enables it rather than using a separate overlay state. Removes the duplicate re-mint logic, the dedicated `overlay-legend` template block, and `useModelOverlay` entirely. Blocks `WANT-1` (models need to render before the modeling flow is worth exercising end-to-end). See `tasks/WANT-12.md`.
