...
export interface OfflineUsage {
  ...
  shell?: { bytes: number, count: number }
  data?: { bytes: number, count: number }
...
const usage = ref<OfflineUsage | null>(null)
const error = ref('')
...
function ask(message: any, { onProgress = null, timeout = 600000 } = {}): Promise<any> {
...
export function useOffline() {
   ...
   */
  const measured = useState<Record<string, number | null>>('offline-measured', () => ({}))
  ...
  async function run(kind: string, message: any) {
    error.value = ''
    busy.value = kind
    progress.value = { done: 0, total: 0 }
    try {
      const res = await ask(message, {
        onProgress: (p: any) => { progress.value = { done: p.done, total: p.total } },
      })
      await refreshUsage()
      return res
    } catch (err: any) {
  ...
  const saveData = (urls: string[]) => run('data', { type: 'save-data', urls })

  /** Save basemap tiles for an area. */
  const saveTiles = (urls: string[]) => run('tiles', { type: 'save-tiles', urls })
   ...
   */
  async function saveArea({ name, bounds, minZoom, maxZoom, sources, id }: any) {
  ...
  const resaveArea = (area: OfflineArea, sources: any[]) => saveArea(plain({
   ...
   */
  function registerEeTemplate(layerId: string, template: any) {
   ...
   * responses admit to counted the shell and the dataset and almost none of
   * the
