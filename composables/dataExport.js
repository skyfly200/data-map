 ...
 */
export function columnsOf(features = []) {
  const seen = new Set()
 ...
 * formulas shows the apostrophe and one that does shows the text. Stripping the
...
export function quoteCsv(value) {
  ...
  if (typeof value === 'object') {
    // An array or a nested object in a property. JSON keeps it readable and
    // keeps it in one field.
    return quoteCsv(JSON.stringify(value))
  }

  let text = String(value)
  // Only for text that a spreadsheet would evaluate. A bare number reaches the
  // branch above and is never prefixed.
 ...
 */
export function toCsv(features = [], { columns = null, geometry = true } = {}) {
 ...
 */
export function toGeoJson(features = [], { columns = null, pretty = false } = {}) {
  const out = columns
    ? features.map((f) => {
      const props = f?.properties || {}
      const narrowed = {}
...
export function exportFilename(stem, format, { stamp = new Date() } = {}) {
...
export function exportBlob(features, format, options = {}) {
  if (format === 'csv') {
    // The BOM is what makes Excel read the file as UTF-8 rather than as the
    // system codepage. Without it every accented species name arrives mangled,
    // and the people most affected are the ones recording them carefully.
