 ...
 */
export function previewOrder(list, draggingId, overId) {
 ...
 */
export function useDragReorder({ key = 'default', ids, onCommit, onPause, onResume } = {}) {
  const { dragging, over } = stateFor(key)

  const current = () => (typeof ids === 'function' ? ids() : (ids?.value ?? ids ?? []))
