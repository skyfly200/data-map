...
export function useEeTiles() {
  ...
  const catalogue = useState<EeLayer[]>('ee-tiles-catalogue', () => [])
  ...
  const keyFor = (layer: string, params: Record<string, any>) => [layer, ...Object.keys(params || {}).sort()
    .map((k) => `${k}=${params[k]}`)].join('|')

  /** A tile template for one layer at one set of parameters. */
  async function template(layer: string, params: Record<string, any> = {}): Promise<EeTemplate> {
    ...
    const entry: EeTemplate = { ...data, at: Date.now() }
    minted.set(id, entry)
    return entry
  }
   ...
   */
  async function classes(layer: string): Promise<any> {
