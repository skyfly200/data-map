...
import type { ObservationFilters } from '~/composables/useFilters'

const KEY = 'saved-filters'

export interface FilterSnapshot extends Omit<ObservationFilters, 'center' | 'radiusKm'> {
...
export function snapshotFilters(filters: ObservationFilters | null | undefined, species = []): FilterSnapshot {
  const f = filters || {}
  return {
 ...
 */
export function describeFilters(snapshot: FilterSnapshot | null | undefined): string {
  const s = snapshot || {}
  const parts: string[] = []
  if (s.species?.length) {
...
export function useSavedFilters() {
  const subsets = useState<SavedFilter[]>('saved-filters', () => [])
  ...
  function save(name: string | null | undefined): string | null {
  ...
  function rename(id: string, name: string | null | undefined) {
    const label = String(name || '').trim()
    if (!label) return
