...
export function cellKeyFor(lat: number, lon: number, size: number): string {
  return `${Math.floor(lat / size)}_${Math.floor(lon / size)}`
}

/** Every cell key covering a bounding box. */
export function cellsForBounds(bounds: Bounds, size: number): string[] {
  const out: string[] = []
  const y0 = Math.floor(bounds.south / size); const y1 = Math.floor(bounds.north / size)
  const x0 = Math.floor(bounds.west / size); const x1 = Math.floor(bounds.east / size)
...
export function pickCells(
  keys: string[],
  { loaded, index, centre, size, limit = MAX_CELLS_PER_MOVE }: {
    loaded: Set<string>
    index: any
    centre?: [number, number]
    size: number
...
const loadedCells = ref(new Set<string>())
const pending = ref(0)
const failed = ref(0)
...
const byId = shallowRef(new Map<string, ObservationFeature & { __full?: boolean }>())
const version = ref(0)
...
function absorb(features: ObservationFeature[], { full }: { full: boolean }) {
...
export function useMapChunks() {
   ...
   *
   * Zoomed out past DETAIL_ZOOM the viewport spans too much ground than is worth
   ...
   */
  async function loadForView(view: View) {
    await loadIndex()
    if (!available.value || !view.bounds) return
    if (view.zoom < DETAIL_ZOOM) return
    const size = index.value.cellSize
    const keys = cellsForBounds(view.bounds, size)
    const next = pickCells(keys, { loaded: loadedCells.value, index: index.value, centre: view.centre, size })
