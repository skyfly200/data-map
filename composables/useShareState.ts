...
export function useShareState() {
   ...
   */
  function collect({ mapView = null, colorBy = '', sizeBy = '', extra = null }: ShareStateOptions = {}): ShareParams {
    const q: ShareParams = {}
   ...
   */
  function apply(query: ShareParams = {}): ApplyResult {
    const num = (v: any) => {
      const n = Number(v)
  ...
  function buildUrl(state: ShareStateOptions = {}, path: string | null = null): string {
  ...
  function buildEmbedUrl(state: ShareStateOptions = {}, path: string | null = null): string {
    const url = new URL(buildUrl(state, path), import.meta.client ? window.location.origin : 'https://example.com')
    url.searchParams.set('embed', '1')
    return url.toString()
  }

  function buildEmbedCode(state: ShareStateOptions = {}, path: string | null, { width = '100%', height = 520 } = {}) {
