...
const SETTINGS_KEYS = [
  ...
  'dashboard-config',           // the dashboard's widgets and their order
]
...
export function chartsToRows(charts = [], userId: string) {
 ...
 */
export function explainSyncError(err: any): SyncError {
...
export function useCloudSync() {
  ...
  // is a valid, complete state, not a degraded one.
  const status = useState<'off' | 'syncing' | 'synced' | 'error'>('cloud-sync-status', () => 'off')
  const error = useState('cloud-sync-error', () => '')
  const errorHint = useState('cloud-sync-hint', () => '')
  const errorRaw = useState('cloud-sync-raw', () => '')
  const lastSynced = useState<string | null>('cloud-sync-at', () => null)
