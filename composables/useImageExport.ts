...
function inlineStyles(source: SVGSVGElement, clone: SVGSVGElement) {
  ...
  for (let i = 0; i < from.length; i++) {
    const computed = getComputedStyle(from[i] as Element)
    let css = ''
...
export function useImageExport() {
   ...
   */
  function serializeSvg(svg: SVGSVGElement, options: SvgOptions = {}): SvgResult {
  ...
  function svgBlob(svg: SVGSVGElement, options: SvgOptions = {}) {
  ...
  async function svgToPng(svg: SVGSVGElement, { scale = 2, background = '#ffffff' } = {}) {
   ...
   */
  async function mapToPng(container: HTMLElement, { scale = 2, background = '#ffffff' } = {}) {
    ...
    for (const el of layers) {
      ...
      try {
        if (el.tagName === 'SVG' || el.tagName === 'svg') {
          const { markup } = serializeSvg(el as SVGSVGElement, { background: null })
          const img = new Image()
          await new Promise<void>((res) => { img.onload = res; img.onerror = res; img.src = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(markup)}` })
  ...
  function slugify(name: string, fallback = 'export'): string {
