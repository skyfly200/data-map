...
const registry = shallowRef<ShortcutEntry[]>([])
...
export function eventKeys(e: KeyboardEvent): string {
  ...
  // Shift is recorded for letters and named keys, but NOT for other
...
export function useShortcuts() {
   ...
   */
  function register(items: Array<{ keys: string; label: string; scope: string; run: (e: KeyboardEvent) => void }>) {
    const entries: ShortcutEntry[] = items.map((i) => {
  ...
  const grouped = computed(() => {
    const byScope = new Map<string, ShortcutEntry[]>()
  ...
  /** The key for an action, a for a tooltip — empty if it has none. */
  function keyFor(scope: string, label: string): string {
    return registry.value.find((s) => s.scope === scope && s.label === label)?.keys || ''
  }

  /** "Show my location" → "Show my location (L)". */
  function withKey(text: string, keys: string | null): string {
