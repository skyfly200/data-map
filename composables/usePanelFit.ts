 ...
 */
export function fitOffset(rect: DOMRect | null, viewportWidth: number, edge = EDGE): number {
  if (!rect) return 0
  let dx = 0
 ...
 */
export function usePanelFit(panel: Ref<HTMLElement | null>, isOpen: Ref<boolean>) {
  const shift = ref(0)
