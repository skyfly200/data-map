...
export const ALL_NUMERIC: NumericField[] = [
...
const BOUNDS = new Map<string, [number | null, number | null]>(
  ALL_NUMERIC.filter((f) => f.bounds).map((f) => [f.key, f.bounds!])
)

/** Physical bounds for a field key, or null when it has none. */
export function boundsFor(key: string): [number | null, number | null] | null {
...
// compared at all.
export const TAXON_RANKS: CategoryField[] = [
...
export const ALL_CATEGORY: CategoryField[] = [
  ...TAXON_RANKS,
 ...
 *
 * `valueOf` reads the number a "by value" sort should use — the measure for
 * a bar, the sample count for a box plot.
 */
export function sortEntries(
  entries: ChartEntry[],
  mode: string,
  valueOf = (e: ChartEntry) => e.value
): ChartEntry[] {
  const list = [...(entries || [])]
  // `numeric` so "Cluster 10" sorts after "Cluster 9" rather than before it.
  const byLabel = (a: ChartEntry, b: ChartEntry) =>
