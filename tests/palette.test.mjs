...
test('every palette can color a full legend without repeating', () => {
  // An eight-color palette made repeated swatches unavoidable in a key whose
  ...
  for (const p of PALETTES) {
    assert.ok(p.colors.length >= LEGEND_CAP,
      `${p.key} has ${p.colors.length} colors for a ${LEGEND_CAP}-row legend`)
    assert.equal(new Set(p.colors).size, p.colors.length, `${p.key} repeats a color`)
  }
})

test('no palette contains a color that disappears into the map', () => {
  ...
  for (const p of PALETTES) {
    for (const c of p.colors) {
      assert.match(c, /^#[0-9a-f]{6}$/i, `${p.key}: ${c} is not a hex color`)
...
test('shading leaves anything that is not a plain hex color alone', () => {
