...
test('the same value always gets the same color', () => {
  // This is the whole point of stableColor: a species keeps its color across
...
test('missing values fall to the neutral color rather than a palette slot', () => {
...
test('cluster labels are read through their prefix', () => {
  // Live clusters are labelled "K3"/"C3"; they must land on the same color as
...
test('an unparseable cluster label is neutral, not a wrong color', () => {
...
test('every palette is well-formed and distinct', () => {
  ...
  for (const p of PALETTES) {
    assert.ok(p.colors.length >= 8, `${p.key} needs at least 8 colors`)
    for (const c of p.colors) {
      assert.match(c, /^#[0-9a-f]{6}$/i, `${p.key} has a non-hex color: ${c}`)
    }
    // Repeats inside one palette would make two categories indistinguishable.
    assert.equal(new Set(p.colors).size, p.colors.length, `${p.key} repeats a color`)
    // The neutral "no value" color must never be a category color.
