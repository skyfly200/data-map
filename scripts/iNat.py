...
def _taxa_lookup(ids):
    """/v1/taxa/{ids}, through the same throttled session as the observations.

    The rate limit is per API, not per endpoint. Left on the default session
    these lookups would run alongside the observation fetches without either
    knowing about the other, which is exactly the uncoordinated bursting the
    shared session exists to stop.
    """
    session = _inat_session()
    if session is not None:
        return get_taxa_by_id(ids, session=session)
    return get_taxa_by_id(ids)


def fetch_taxa(taxon_ids, batch_size=30, fetcher=_taxa_lookup):
...
def resolve_taxonomy(rows, fetcher=_taxa_lookup):
